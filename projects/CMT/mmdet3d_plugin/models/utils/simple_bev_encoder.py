...
class SimpleBEVEncoder(nn.Module):
    ...
    def bev_projection(self,
        ...
        img_feats = mlvl_feats[0] ### Assume we only use single level of features for simplicity
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        reference_points_3d = self.get_bev_reference_points(B=B, dtype=dtype, device=device)
        reference_points_cam, bev_mask = self.point_sampling(reference_points_3d, img_metas)
        bev_feats = self.bev_projection(reference_points_cam, mlvl_feats, bev_mask)

        bev_feats = self.bev_encoder(bev_feats.view(B, self.H_bev, self.W_bev, -1).permute(0, 3, 1, 2))  # [B, C, H_bev, W_bev]

        bev_feats = bev_feats.permute(0,2,3,1).view(B, self.H_bev*self.W_bev, -1) # [B, C, H_bev*W_bev] -> [B, H_bev*W_bev, C]
