...
from projects.CMT.mmdet3d_plugin.models.utils.grid_mask import GridMask
from projects.CMT.mmdet3d_plugin import SPConvVoxelization
...
class CmtDetector(MVXTwoStageDetector):
    ...
    def _freeze_backbone(self):
        self.train()
        for modules in [self.img_backbone,
                        self.img_neck,
                        self.pts_backbone,
    ...
    def _freeze_detector(self):
        ...
        for modules in [self.pts_bbox_head.timestamp_encoder,
                        self.pts_bbox_head.bev_pose_encoder,
                        self.pts_bbox_head.bev_embed_proj,
                        self.pts_bbox_head.simple_bev_encoder,
                        self.pts_bbox_head.learned_vel_estimator]:
            if modules is not None:
                modules.train()
                for param in modules.parameters():
    ...
    def forward_pts_train(self,
        ...
        if img_feats is None:
            img_feats = [None]
        outs, flow_outs= self.pts_bbox_head(pts_feats, img_feats, img_metas)
        loss_inputs = [gt_bboxes_3d, gt_labels_3d, outs]
        losses = self.pts_bbox_head.loss(*loss_inputs, flow_output=flow_outs)
        return losses
    ...
    def simple_test_pts(self, x, x_img, img_metas, rescale=False):
        ...
        outs, flow_outs = self.pts_bbox_head(x, x_img, img_metas)
